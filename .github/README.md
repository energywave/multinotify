...
| `alexa_type`   | Alexa      | You can specify `announce` or `tts` to use a specific type of announce. If not specified `announce` will be used. (please refer to the [Alexa Media Player](https://github.com/custom-components/alexa_media_player/wiki/Configuration%3A-Notification-Component#functionality) documentation)
